# 🤖 Local AI Agent v1.2.0 (Chrome to OS Bridge)
...
As of **v1.2.0**, the architecture has been massively upgraded to a **Full-Duplex Persistent Connection**. It not only executes local Python scripts safely but now supports **real-time interactive terminal input (`input()`)**, alongside automatically feeding execution results and error tracebacks back to the AI for self-correction. Your web AI is now a fully autonomous, interactive local execution agent!

## ✨ Features

- **⌨️ Interactive Terminal I/O (NEW in v1.2.0):** Scripts are no longer static! The agent now fully supports Python's native `input()` function. When the script requires human interaction, execution pauses, and a sleek text box dynamically renders beneath the code block in your browser. Type your answer, press Enter, and the data is seamlessly piped back to the local Python process.
- **🔌 Full-Duplex Persistent Connection (NEW in v1.2.0):** Upgraded from a single-shot request to a persistent, robust communication pipeline. The local Python host now maintains a steady event loop, allowing for complex, multi-step code execution without dropping the connection. Includes built-in crash detection to prevent UI freezing if the execution fatally fails.
...
This project implements a seamless Inter-Process Communication (IPC) architecture with an advanced feedback and interactive IO engine:

1. **Frontend (Chrome Extension):** Uses a Singleton Lock to prevent duplicate UI injections. It captures code from the AI's DOM, cleans it, and establishes a persistent full-duplex pipeline via `chrome.runtime.connectNative`. It dynamically manages DOM injections for `input()` prompts.
...
1. Ask ChatGPT, Claude, or DeepSeek to write a Python script (e.g., *"Write a script that asks for my name and creates a text file on my desktop greeting me"*).
2. You will see a green `🚀 Run in Local` button instantly appear on the AI's code block.
3. Click it. If the code contains an `input()` statement, a blue **Input Box** will appear right under the code block.
4. Type your response, hit Enter, and watch the execution continue.
5. **The Feedback Loop:** Once the code finishes running (or if it crashes), the output/error log will magically appear inside your chat input box. Just press **Enter** to send it back to the AI for the next step or automatic debugging!
